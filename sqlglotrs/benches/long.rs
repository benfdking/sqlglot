...
//     use crate::benchmark::LONG;
//
//     #[bench]
//     fn bench_parsing_long(b: &mut Bencher) {
//         // Read tokenizer settings
//         let settings_file = std::fs::read_to_string("/Users/benjaminking/Developer/sqlglot/.hacking/tokenizer_settings.json").unwrap();
//         let tokenizer_settings = serde_json::from_str::<TokenizerSettings>(&settings_file).unwrap();
//
//         let settings_type_file = std::fs::read_to_string("/Users/benjaminking/Developer/sqlglot/.hacking/token_type_settings.json").unwrap();
//         let settings_type_file = serde_json::from_str::<TokenTypeSettings>(&settings_type_file).unwrap();
//
//         let dialect_settings = std::fs::read_to_string("/Users/benjaminking/Developer/sqlglot/.hacking/dialect_settings.json").unwrap();
//         let dialect_settings = serde_json::from_str::<TokenizerDialectSettings>(&dialect_settings).unwrap();
//
//         let tokenizer = Tokenizer::new(tokenizer_settings, settings_type_file);
//
//         b.iter(|| {
//             tokenizer.tokenize(LONG, &dialect_settings);
//         })
//     }
// }
